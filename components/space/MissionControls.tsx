...
export function MissionControls({ config, onConfig, paused, onPaused, speed, onSpeed, follow, onFollow, localView, onLocalView, onReset, missionMode, onMissionMode }: {
  ...
  missionMode: MissionMode; onMissionMode: (value: MissionMode) => void;
}) {
  const locked = missionMode !== 'free';
  ...
  return (
    <aside className="mission-panel">
      ...
      <div className="scenario-tabs" aria-label="Choix du scénario">
        ...
        <button className={missionMode === 'juice' ? 'active' : ''} onClick={() => onMissionMode('juice')}>JUICE · ESA</button>
        <button className={missionMode === 'free' ? 'active' : ''} onClick={() => onMissionMode('free')}>Libre</button>
      </div>
      {missionMode === 'voyager2' && <div className="historical-note"><strong>Grand Tour · 1977–1989</strong><span>Jupiter → Saturne → Uranus → Neptune</span></div>}
      {missionMode === 'juice' && <div className="historical-note esa-note"><strong>Mission européenne · 2023–2031</strong><span>Lune–Terre → Vénus → Terre → Terre → Jupiter</span></div>}
