...
const JUICE_CONFIG: SimulationConfig = { departureId: 'earth', targetId: 'jupiter', assistId: 'earth', departureDate: JUICE_LAUNCH, deltaV: 2.5 };

export default function Home() {
  const [missionMode, setMissionMode] = useState<MissionMode>('voyager2');
  ...
  useEffect(() => {
    ...
    let carry = 0;
    const samplesPerDay = missionMode === 'free' ? 24 : 1;
  ...
  const reset = () => { setConfig(missionMode === 'voyager2' ? VOYAGER_CONFIG : missionMode === 'juice' ? JUICE_CONFIG : FREE_CONFIG); setIndex(0); setPaused(true); setSpeed(10); setFollow(missionMode !== 'free'); setLocalView(false); };
  const changeMissionMode = (mode: MissionMode) => {
    setMissionMode(mode); setConfig(mode === 'voyager2' ? VOYAGER_CONFIG : mode === 'juice' ? JUICE_CONFIG : FREE_CONFIG); setIndex(0); setPaused(true); setFollow(mode !== 'free'); setLocalView(false);
  };
  const replayFlyby = () => {
  ...
  return (
    <main className="app-shell">
      <header className="topbar">
        ...
        <div className="mission-title"><span>{missionMode === 'voyager2' ? 'Mission historique · Grand Tour' : missionMode === 'juice' ? 'Mission européenne · ESA' : 'Simulation libre'}</span><h1>{missionMode === 'voyager2' ? <>Voyager 2 · Terre <b>→</b> Jupiter <b>→</b> Saturne <b>→</b> Uranus <b>→</b> Neptune</> : missionMode === 'juice' ? <>JUICE · Terre <b>→</b> Vénus <b>→</b> Terre <b>→</b> Jupiter</> : <>{getBody(config.departureId).name} <b>→</b> {getBody(config.assistId).name} <b>→</b> {getBody(config.targetId).name}</>}</h1></div>
